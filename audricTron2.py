...
filename = "model_keras_audric_d.keras"
...
def create_model():
    ...
    model.add(Input(shape=(input_size, 1), name='input'))
...
def train_model():
    ...
    if p1_mode in ["ai_retrain", "ai_train_random"] or p2_mode in ["ai_retrain", "ai_train_random"]:
        x_feedback = np.array(training_input_data).reshape(-1, input_size, 1)
        y_feedback = np.array(training_feedback_data).reshape(-1, 1)
        model.fit(x_feedback, y_feedback, epochs=25, shuffle=True)
...
def load_model_from_disk():
    ...
    if p1_mode in ["ai_load_trained", "ai_retrain"] or p2_mode in ["ai_load_trained", "ai_retrain"]:
        try:
            ...
            print("Modèle chargé avec succès depuis le disque.")
...
def determine_score():
    ...
    if (p2x == p1x and p2y == p1y) or (p2x == p1x_prev and p2y == p1y_prev) or (p2x_prev == p1x and p2y == p1y) or\
...
def analyze_game_state():
    ...
    if grid[p2_temp_x][p2_temp_y] > 0:
        ob[1][2] = 1
    sur_blocks = np.zeros((2, visible_block_range ** 2 - 1))
    get_surroundings()
    # print()
